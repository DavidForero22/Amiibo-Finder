...
- Motion is quick ease-out at rest; the reveal and the fly-to-shelf are the only choreographed sequence. Small physical touches keep the room alive (figures hop under the hand, the lamp flickers on at night, cards turn over), and every overlay leaves the way it arrived.
...
The Collection route follows the stage with the ledger, separated by clamp(40px, 7vw, 88px) and opened by a 2px ink rule. The ledger grid is `auto-fill, minmax(176px, 1fr)` with tiles on a two-row subgrid so cards and names align across a row; below 480px it is exactly two columns. Controls sit in a wrapping toolbar (search grows from 280px, filters panel fields from 200px).
...
The dialog becomes a wall of the room: a bottle-green gradient with a walnut floor strip along the bottom 20%, a 3px brass ring. Left, the figure rises (900ms settle) under a warm brass spotlight onto a soft plinth shadow; right, the name wipes in left to right at Display size in cream (`reveal-ink`), a walnut description plate with brass text, and the brass "Put it on the shelf" button. Focus rings inside switch to polished brass. Under 760px it stacks with the floor strip under the figure. On "Put it on the shelf" the figure (view-transition-name `arriving-figure`) flies to its shelf slot over 720ms (`cubic-bezier(0.5, 0, 0.2, 1)`) while the page hands over (old out in 180ms, new in over 240ms after 120ms), then lands with a 700ms settle bounce. With reduced motion, or without View Transitions support, the dialog simply closes and the figure appears. Confetti on unlock uses the room's colors (brass, cobalt wrap, bottle green, paper) and is skipped entirely under reduced motion.

### Gift Box
An isometric SVG box on the rug that is the primary button on Unlock. Ready: a 3.4s idle hop with a squash and a bow wiggle, paused and lifted 3% on hover or focus. Opening: the lid pops off (700ms) and pale gold rays rise from inside. Waiting: an empty spot on the rug outlined by a marching dashed brass ring. Box and spot share the view-transition name `delivery`, so moving between Collection and Unlock fades them out or in (380ms) while the room (`room`) glides to its new place; they also fade in on mount where View Transitions are unsupported.

### Room Life
Shelf figures hop once when the pointer passes over them (480ms: squash, 13% lift, small second bounce, contact shadow shrinking), hover-capable pointers only. Switching day and night (only on a user toggle, via a 1s `theme-switching` class on `<body>`) sinks the outgoing sun or moon out of the window pane and raises the other in (240ms out, 360ms in after 140ms, clipped to the glass), eases the sky gradient over 480ms, flickers the floor lamp on over 900ms (light pool, bulb and shade together) and dims it off over 420ms with one last dip.

### Ledger Tile
A two-sided card per figure (3:4, growing to fit its back on narrow columns). Front: a display box with a bottle-green back wall lit from above, a walnut floor strip in the bottom 16%, a 1.5px brass inset edge (2.5px polished brass when a favorite), the figure standing on the strip with a drop shadow and rising 4px on hover, and a small brass info mark on the strip. Back: the record card on paper stock (`paper`, local tag ink) with a punched hole at the top, listing unlock date, amiibo series and release per region (dt over dd). A transparent button over the whole card (`aria-expanded`, controls the back) turns it over with a 640ms `rotateY(180deg)` on the settle curve; hovering tilts it 10deg as a hint. A 40px round favorite button sits top right on a dark translucent disc, above the flip control, and turns polished brass with a 420ms pop when pressed (`aria-pressed`); it fades out while the back is showing. Below the card: the name (Title) and the series (soft ink).
...
### Navigation
Bold soft-ink links, 44px tall, 10px radius; hover gives ink and a surface fill; the active route turns ink with a 3px brass underline that scales in from the center. The brand is an isometric cobalt gift mark (34px) that tips -6deg on hover beside the Anybody wordmark at width 120. The user menu is a raised dropdown (230px min, Float shadow) with 44px items, a divider, and a danger-tinted clear item. The language picker is a compact face showing the code (EN/ES/FR) with a chevron that turns when open; it opens the same dropdown as a radio menu (190px min), each language named in itself with a cobalt check on the current one. Both menus share one ARIA menu-button model and animate both ways: 200ms drop-and-scale in, 160ms fold back out before unmounting. The filters panel folds its height open (260ms) and closed (220ms) so the ledger glides instead of jumping.
...
- **Do** keep controls at least 44px tall and use native elements (`<dialog>`, buttons with `aria-expanded`, checkbox inputs) for their built-in keyboard behavior.
